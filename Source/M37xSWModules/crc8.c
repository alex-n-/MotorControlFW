...
static const uint8_t  uint8_table[256] =
{
...
uint8_t crc_update(uint8_t crc, uint8_t* data, int data_len)
{
  ...
  while(data_len--)
  {
    ...
    crc = (uint8_table[tbl_idx] ^ (crc << 8)) & 0xff;
...
{
  return crc_update(0x00, data, len) ^ 0x00;
}
