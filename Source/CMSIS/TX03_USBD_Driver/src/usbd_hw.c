...
#define USBD_DMACCONFIG_MASK_READ     (uint32_t)0x0155U
#define USBD_DMACCONFIG_MASK_WRITE    (uint32_t)0x0177U

#define USBD_DMACSTATUS_MASK    (uint32_t)0x001FU
#define USBD_POWERCTRL_MASK     (uint32_t)0x00FFU
#define UDC2_EP0STATUS_MASK     (uint32_t)0xBE00U
#define UDC2_EPxSTATUS_MASK     (uint32_t)0xFF8FU
...
/**
  * @brief  Set USBPLL (multiplier circuit) operation.
  * @param  OnOff: Specify the 'On' or 'Off' operation.
  *    This parameter can be one of the following values:
  *      USBD_OFF, USBD_ON
  * @retval None
  */
void USBD_SetPLL(USBD_ONOFF OnOff)
{
    assert_param(IS_USBD_ONOFF(OnOff));
    if( OnOff == USBD_ON ) { 
        TSB_USBPLL_CR_USBPLLON = 1U; 
    } else {
        TSB_USBPLL_CR_USBPLLON = 0U; 
    }    
}


/**
  * @brief  Set whether feed 48M clock to USB-Device module or not.
  * @param  OnOff: Specify the 'On' or 'Off' operation.
  *    This parameter can be one of the following values:
  *      USBD_OFF, USBD_ON
  * @retval None
  */
void USBD_SetDeviceClock(USBD_ONOFF OnOff)
{
    assert_param(IS_USBD_ONOFF(OnOff));
    if( OnOff == USBD_ON ) { 
        TSB_USBPLL_EN_USBDEN = 1U; 
    } else {
        TSB_USBPLL_EN_USBDEN = 0U; 
    }    
}

/**
  * @brief  Set a value of PLL multiplier for USB.
  * @param  Multiplier: Specify the multiplier value for corresponding clock input.
  *    This parameter can be one of the following values:
  *     USBD_INPUT_EXT  : Set to use external 48M USBECLK for USB    
  *     USBD_INPUT_8M,  : Set to use 8M  feosc and PLL clock for USB
  *     USBD_INPUT_12M, : Set to use 12M feosc and PLL clock for USB  
  *     USBD_INPUT_16M  : Set to use 16M feosc and PLL clock for USB 
  *       Note: feosc means EHCLKIN or EHOSC .
  * 	        EHCLKIN : External high-speed oscillation clock from X1 clock input 
  * 	        EHOSC : External high-speed oscillation clock from X1 and X2 oscillator connected   
  * @retval None
  */
void USBD_SetPLLMultiplier(USBD_PLLMULTIPLIER Multiplier)
{
    assert_param(IS_USBD_PLLMULTIPLIER(Multiplier));

    TSB_USBPLL->SEL = Multiplier;
}
