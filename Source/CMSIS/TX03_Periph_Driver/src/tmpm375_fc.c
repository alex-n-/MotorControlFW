...
/** @defgroup FC_Private_Defines
  * @{
  */
#define CR_FLBOFF_SET          ((uint32_t)0x00000000)
#define CR_FLBOFF_CLEAR        ((uint32_t)0x00000001)
...
#define FC_ERASE_CHIP_OVER_TIME             ((uint32_t)0x0007FFFF)      /* FC_ERASE_CHIP_OVER_TIME > 0.2s */
...
/**
  * @brief  Set the value of CR register.
  * @param  NewState: The value of CR register.
...
{
    uint32_t tmp = TSB_FC->CR;
    /* Check the parameters */
    assert_param(IS_FUNCTIONAL_STATE(NewState));

    if (NewState == ENABLE) {
        /* Set CR<FLBOFF> to "0" that enable security function  */
        tmp &= CR_FLBOFF_SET;
    } else {
        /* Set CR<FLBOFF> to "1" that disable security function  */
        tmp |= CR_FLBOFF_CLEAR;
    }
    TSB_FC->CR = tmp;
}
...
FC_Result FC_ProgramBlockProtectState(uint8_t BlockNum)
{
    ...
    volatile uint32_t counter = FC_SET_PROTECT_STATE_OVER_TIME;
...
/**
  ...
  *   This parameter can be one of the following values:
  *   FC_BLOCK_GROUP_0: block 0 through 1.
...
FC_Result FC_WritePage(uint32_t PageAddr, uint32_t * Data)
{
    ...
    volatile uint32_t counter = FC_WRITE_PAGE_OVER_TIME;
    uint32_t i = 0U;
