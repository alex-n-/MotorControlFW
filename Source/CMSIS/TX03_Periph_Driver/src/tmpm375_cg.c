...
Result CG_SetFcSrc(CG_FcSrc Source)
{
    ...
    FunctionalState fosc_osc1 = DISABLE;
    FunctionalState fosc_osc2 = DISABLE;

    /* Check the parameters */
    assert_param(IS_CG_FC_SRC(Source));

    fosc_osc1 = CG_GetFoscState(CG_FOSC_OSC1);
    fosc_osc2 = CG_GetFoscState(CG_FOSC_OSC2);

    if ((fosc_osc1 == ENABLE) || (fosc_osc2 == ENABLE)) {
        fosc_on = ENABLE;
    } else {
        /* Do nothing */
    }
                             ...
                             CG_INTActiveState ActiveState, FunctionalState NewState)
{
    ...
    regval.byte[num] &= 0x8EU;
    regval.byte[num] |= ActiveState;
    TSB_CG->IMCGA = regval.byte4;
    regval.byte[num] |= NewState;
    TSB_CG->IMCGA = regval.byte4;
}
