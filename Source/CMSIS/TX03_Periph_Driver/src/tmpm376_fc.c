...
/* Includes ------------------------------------------------------------------*/
#include "tmpm376_fc.h"

#if defined(__TMPM376_FC_H) || defined(__TMPM377_FC_H)
...
#define FC_WRITE_PAGE_OVER_TIME             ((uint32_t)0x00000FFF)      /* FC_WRITE_PAGE_OVER_TIME > 1.28ms */
#define FC_SET_PROTECT_STATE_OVER_TIME      ((uint32_t)0x000FFFFF)

#define FLASH_AREA                  ((uint32_t)(FLASH_START_ADDR & 0xFFF80000U))

#define FC_PBA_BLOCK_0              ((uint32_t *) (FLASH_AREA + 0x00010000U))
#define FC_PBA_BLOCK_1              ((uint32_t *) (FLASH_AREA + 0x00010200U))
#define FC_PBA_BLOCK_2              ((uint32_t *) (FLASH_AREA + 0x00010400U))
#define FC_PBA_BLOCK_3              ((uint32_t *) (FLASH_AREA + 0x00010600U))
...
uint8_t FC_AddrToBlockNum(uint32_t Addr)
{
    ...
    case 7U:
        BlockNum = FC_BLOCK_2;
        break;
#if defined(TSB_M376FD) || defined(TSB_M377FD)
    case 8U:
    case 9U:
    case 10U:
    case 11U:
...
/**
  ...
  *   This parameter can be one of the following values:
  *   FC_BLOCK_0(for M376FD,M377FD), FC_BLOCK_1(for M376FD,M377FD),
  *   FC_BLOCK_2, FC_BLOCK_3, FC_BLOCK_4, FC_BLOCK_5.
  * @retval DISABLE or ENABLE.
  *
  */
FunctionalState FC_GetBlockProtectState(uint8_t BlockNum)
{
    ...
    tmp = TSB_FC->FLCS & (FLCS_BLOCK0_MASK << BlockNum);
...
/**
  ...
  *         The data size should be same as page size.
  * @retval FC_SUCCESS, FC_ERROR_PROTECTED, FC_ERROR_OVER_TIME.
  *
  */
...
#endif                          /* defined(__TMPM376_FC_H) || defined(__TMPM377_FC_H) */
